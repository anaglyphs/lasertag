...
namespace Anaglyph.DepthKit
{
	public class Marcher : MonoBehaviour
	{
		...
		private MeshUpdateFlags flags = MeshUpdateFlags.DontNotifyMeshUsers | MeshUpdateFlags.DontRecalculateBounds |
		...
		public void TriangulateVoxelRange(uint3 start, uint3 end, Mesh mesh)
		{
			...
			for (uint z = start.z; z < end.z - 1; z++)
				TriangulateVoxel(new uint3(x, y, z));
		...
		private void TriangulateVoxel(uint3 coord)
		{
			uint c0 = CoordToIndex(coord + new uint3(0, 0, 0));
			uint c1 = CoordToIndex(coord + new uint3(1, 0, 0));
			uint c2 = CoordToIndex(coord + new uint3(1, 0, 1));
			uint c3 = CoordToIndex(coord + new uint3(0, 0, 1));
			uint c4 = CoordToIndex(coord + new uint3(0, 1, 0));
			uint c5 = CoordToIndex(coord + new uint3(1, 1, 0));
			uint c6 = CoordToIndex(coord + new uint3(1, 1, 1));
			uint c7 = CoordToIndex(coord + new uint3(0, 1, 1));

			float3 p0 = CoordsToPos(c0);
			float3 p1 = CoordsToPos(c1);
			float3 p2 = CoordsToPos(c2);
			float3 p3 = CoordsToPos(c3);
			float3 p4 = CoordsToPos(c4);
			float3 p5 = CoordsToPos(c5);
			float3 p6 = CoordsToPos(c6);
			float3 p7 = CoordsToPos(c7);
			...
			int[] t = triTable[cubeindex];
			foreach (int i in t)
				tris.Add((uint)(i + triCount));
		}

		private void AddVert(float3 p0, float v0, float3 p1, float v1)
		{
			float lerp = (IsoLevel - v0) / (v1 - v0);
			float3 p = p0 + lerp * (p0 - p1);
			verts.Add(p);
		}
	}
}