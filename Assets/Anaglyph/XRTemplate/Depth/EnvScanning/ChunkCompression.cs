...
namespace Anaglyph.DepthKit.EnvScanningV2
{
	public static class ChunkCompression
	{
		...
		{
			Run, // consecutive values that are the same
			Literal // consecutive values that are all different
		}
		...
		private struct CompressJob : IJob
		{
			...
			private const int MaxTokenLen = 128;

			// index of the pending literal's control byte, backpatched on flush
			private int literalControlIdx;
			private int literalLen;
