...
namespace Anaglyph.XR.DepthKit.EnvScanning.Editor
{
	/// <summary>
	...
	public class EnvScannerDebugWindow : EditorWindow
	{
		...
		private UnityEditor.Editor chunkEditor; // stock Texture3DInspector over chunkTex
		private UnityEditor.Editor atlasEditor; // stock RenderTextureEditor over the live scanner.ChunkData
		...
		private void OnGUI()
		{
			...
			UnityEditor.Editor previewEditor = GetActiveEditor(scanner);
		...
		private UnityEditor.Editor GetActiveEditor(EnvScanner scanner)
		{
			if (mode == ViewMode.SingleChunk)
		...
		private UnityEditor.Editor GetEditor(Texture tex, ref UnityEditor.Editor editor)
		{
			if (tex == null) return null;
			if (editor == null || editor.target != tex)
			{
				if (editor != null) DestroyImmediate(editor);
				editor = UnityEditor.Editor.CreateEditor(tex);
			}
