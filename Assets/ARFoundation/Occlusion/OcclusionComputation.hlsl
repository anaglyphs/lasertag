...
{
	return SAMPLE_TEXTURE2D_ARRAY(_EnvironmentDepthTexture, sampler_EnvironmentDepthTexture, uv,
	                              unity_StereoEyeIndex).r;
}
...
float GetSoftPixelVisibility(const float4 depthSample, const float linearSceneDepth)
{
	...
	float3 minMaxMidEnvDepths = float3(1.0f - depthSample.x, 1.0f - depthSample.y,
	                                   depthSample.z + 1.0f - depthSample.x);
...
float GetToleranceBasedPixelVisibility(const float linearEnvironmentDepth, const float linearSceneDepth)
{
	...
	const float tolerance_t = NormalizeWithinBounds(linearEnvironmentDepth, depthCloseToleranceThreshold,
	                                                depthFarToleranceThreshold);
...
void GetOcclusionAlpha_float(const float3 positionWS, out float alpha)
{
	if (_IsOcclusionOn == 0)
	{
		return;
	...
	const float4 clipSpaceDepthRelativePos = mul(_EnvironmentDepthProjectionMatrices[unity_StereoEyeIndex],
	                                             float4(positionWS, 1.0f));
	...
	alpha = ComputePixelVisibility(environmentDepth, linearSceneDepth);
}
...
void SetOcclusion_float(const float3 positionWS, float4 color, out float4 resultColor)
{
	...
	float alpha;
	GetOcclusionAlpha_float(positionWS, alpha);
