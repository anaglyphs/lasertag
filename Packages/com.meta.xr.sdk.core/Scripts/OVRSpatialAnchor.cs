...
[HelpURL(
	"https://developer.oculus.com/documentation/unity/unity-spatial-anchors-persist-content/#ovrspatialanchor-component")]
...
public partial class OVRSpatialAnchor : MonoBehaviour
{
	...
	public event Action<OperationResult> OnLocalize
	{
		add
		{
			if (Created && OVRPlugin.GetSpaceComponentStatus(_anchor.Handle, OVRPlugin.SpaceComponentType.Locatable,
				    out bool isEnabled, out bool isPending) && !isPending)
	...
	{
		while (this && !Created && !_creationFailed) await Task.Yield();
	...
	public bool Localized => Created &&
	                         OVRPlugin.GetSpaceComponentStatus(_anchor.Handle, OVRPlugin.SpaceComponentType.Locatable,
		                         out bool isEnabled, out _) && isEnabled;
	...
	{
		if (!await WhenCreatedAsync()) return false;

		while (OVRPlugin.GetSpaceComponentStatus(_anchor.Handle, OVRPlugin.SpaceComponentType.Locatable,
			       out bool isEnabled, out bool changePending))
		{
			if (!changePending) return isEnabled;
	...
	{
		List<OVRSpaceUser> userList = OVRObjectPool.List<OVRSpaceUser>();
		userList.Add(user);
	...
	public OVRTask<OVRResult<OVRAnchor.ShareResult>> ShareAsync(Guid groupUuid)
	{
		if (groupUuid == Guid.Empty)
			throw new ArgumentException("groupUuid must not be a 0 uuid", nameof(groupUuid));

		ulong handle = _anchor.Handle;
		unsafe
		{
			ReadOnlySpan<ulong> handleSpan = new(&handle, 1);
			ReadOnlySpan<Guid> groupUuidSpan = new(&groupUuid, 1);
		...
		IEnumerable<OVRSpaceUser> users)
	{
		...
		unsafe
		{
			using OVRNativeList<ulong> spaces = new(anchors.ToNonAlloc().Count, Allocator.Temp);
			foreach (OVRSpatialAnchor anchor in anchors.ToNonAlloc()) spaces.Add(anchor._anchor.Handle);

			using OVRNativeList<ulong> userHandles = new(users.ToNonAlloc().Count, Allocator.Temp);
			foreach (OVRSpaceUser user in users.ToNonAlloc()) userHandles.Add(user._handle);

			OVRPlugin.Result result = OVRPlugin.ShareSpaces(spaces, (uint)spaces.Count, userHandles,
				(uint)userHandles.Count, out ulong requestId);
	...
	/// <exception cref="ArgumentException"> Thrown if <paramref name="groupUuid"/> is <see cref="Guid.Empty"/></exception>
	/// <exception cref="ArgumentNullException">Thrown if <paramref name="anchors"/> is null.</exception>
	public static OVRTask<OVRResult<OVRAnchor.ShareResult>> ShareAsync(IEnumerable<OVRSpatialAnchor> anchors,
		Guid groupUuid)
	{
		if (anchors is null)
			throw new ArgumentNullException(nameof(anchors));
		...
		OVREnumerable<OVRSpatialAnchor> anchorIter = anchors.ToNonAlloc();
		using OVRNativeList<ulong> anchorNativeList = new(anchorIter.Count, Allocator.Temp);
		foreach (OVRSpatialAnchor a in anchorIter) anchorNativeList.Add(a._anchor.Handle);
		unsafe
		{
			ReadOnlySpan<Guid> groupUuidPtr = new(&groupUuid, 1);
		...
		IEnumerable<Guid> groupUuids)
	{
		...
		using OVRNativeList<Guid> groupUuidList = groupUuids.ToNativeList(Allocator.Temp);
		foreach (Guid uuid in groupUuidList)
			if (uuid == Guid.Empty)
				throw new ArgumentException("groupUuids must not contain a 0 uuid", nameof(groupUuids));
	...
	{
		List<OVRSpatialAnchor> shareRequestAnchors = GetListToStoreTheShareRequest(users);
	...
	private List<OVRSpatialAnchor> GetListToStoreTheShareRequest(List<OVRSpaceUser> users)
	{
		...
		foreach ((List<OVRSpaceUser> shareRequestUsers, List<OVRSpatialAnchor> shareRequestAnchors) in ShareRequests)
		{
			if (!AreSortedUserListsEqual(users, shareRequestUsers)) continue;
		...
		// add a new request
		List<OVRSpatialAnchor> anchorList = OVRObjectPool.List<OVRSpatialAnchor>();
	...
	public static OVRTask<OVRResult<OVRAnchor.SaveResult>> SaveAnchorsAsync(IEnumerable<OVRSpatialAnchor> anchors)
	{
		...
		using OVRNativeList<ulong> spaces = new(Allocator.Temp);
		foreach (OVRSpatialAnchor anchor in anchors.ToNonAlloc()) spaces.Add(anchor._anchor.Handle);
		...
		IEnumerable<Guid> uuids)
	{
		...
		using (new OVRObjectPool.ListScope<OVRAnchor>(out List<OVRAnchor> spaces))
		{
			foreach (OVRSpatialAnchor anchor in anchors.ToNonAlloc()) spaces.Add(anchor._anchor);
	...
	private void InitializeUnchecked(OVRSpace space, Guid uuid)
	{
		...
		if (_anchor.TryGetComponent<OVRLocatable>(out OVRLocatable locatable)) locatable.SetEnabledAsync(true);

		if (_anchor.TryGetComponent<OVRStorable>(out OVRStorable storable)) storable.SetEnabledAsync(true);

		if (_anchor.TryGetComponent<OVRSharable>(out OVRSharable sharable)) sharable.SetEnabledAsync(true);
...
#pragma warning restore
	...
	{
		foreach ((List<OVRSpaceUser> userList, List<OVRSpatialAnchor> anchorList) in ShareRequests)
		{
...
#pragma warning restore
	...
	{
		if (_anchor != OVRAnchor.Null) _anchor.Dispose();
	...
	private OVRPose GetTrackingSpacePose()
	{
		...
		return transform.ToOVRPose(false);
	}

	private void CreateSpatialAnchor()
	{
		bool created = OVRPlugin.CreateSpatialAnchor(new OVRPlugin.SpatialAnchorCreateInfo
		{
	...
	{
		bool tryLocateSpace = OVRPlugin.TryLocateSpace(space, OVRPlugin.GetTrackingOriginType(),
			out OVRPlugin.Posef posef, out locationFlags);
		...
		Camera mainCamera = Camera.main;
		if (mainCamera) pose = pose.ToWorldSpacePose(mainCamera);
	...
	{
		return TryGetPose(space, out pose, out _);
	}

	/// <summary>
	/// This is modified! Made public to fix issues with pose timing when using ARFoundation. 
	/// </summary>
	public void UpdateTransform()
	{
		bool hasPose = TryGetPose(_anchor.Handle, out OVRPose pose, out OVRPlugin.SpaceLocationFlags flags);
		IsTracked = hasPose && flags.IsOrientationTracked() && flags.IsPositionTracked();
		if (hasPose) transform.SetPositionAndRotation(pose.position, pose.orientation);
	}
	...
	internal static readonly Dictionary<Guid, OVRSpatialAnchor> SpatialAnchors = new();

	private static readonly Dictionary<ulong, OVRSpatialAnchor> CreationRequests = new();

	private static readonly Dictionary<OVRSpatialAnchor, Guid> AsyncRequestTaskIds = new();

	private static readonly List<(List<OVRSpaceUser>, List<OVRSpatialAnchor>)> ShareRequests = new();

	private static readonly Dictionary<ulong, MultiAnchorDelegatePair> MultiAnchorCompletionDelegates = new();
	...
	{
		if (!MultiAnchorCompletionDelegates.Remove(requestId, out MultiAnchorDelegatePair value)) return;
			...
			foreach (OVRSpatialAnchor anchor in value.Anchors)
				switch (actionType)
				{
					case MultiAnchorActionType.Save:
					{
						...
						if (AsyncRequestTaskIds.Remove(anchor, out Guid taskId))
					...
					case MultiAnchorActionType.Share:
					{
						...
						if (AsyncRequestTaskIds.Remove(anchor, out Guid taskId)) OVRTask.SetResult(taskId, result);
	...
	private static void OnSpatialAnchorCreateComplete(ulong requestId, bool success, OVRSpace space, Guid uuid)
	{
		...
		if (!CreationRequests.Remove(requestId, out OVRSpatialAnchor anchor)) return;

		if (anchor) anchor._creationFailed = !success;
	...
	public readonly partial struct UnboundAnchor
	{
		...
		public bool Localizing => OVRPlugin.GetSpaceComponentStatus(_space, OVRPlugin.SpaceComponentType.Locatable,
			out bool enabled, out bool pending) && !enabled && pending;
		...
		{
			OVRAnchor anchor = new(_space, Uuid);
			if (anchor == OVRAnchor.Null)
				throw new InvalidOperationException(
					$"The {nameof(UnboundAnchor)} is not valid. Was it default (zero) initialized?");

			if (!anchor.TryGetComponent<OVRLocatable>(out OVRLocatable locatable))
				throw new InvalidOperationException($"Anchor {Uuid} is not localizable.");

			if (!locatable.IsEnabled)
				throw new InvalidOperationException(
					$"The anchor {Uuid} is not localized. An anchor must be localized before getting the pose.");

			if (OVRSpatialAnchor.TryGetPose(_space, out OVRPose ovrPose, out _))
			{
		...
		{
			OVRAnchor anchor = new(_space, Uuid);
			if (anchor.TryGetComponent<OVRStorable>(out OVRStorable storable)) storable.SetEnabledAsync(true);

			if (anchor.TryGetComponent<OVRSharable>(out OVRSharable sharable)) sharable.SetEnabledAsync(true);
		...
		Action<List<UnboundAnchor>, int> onIncrementalResultsAvailable = null)
	{
		...
		return LoadUnboundAnchorsAsync(new OVRAnchor.FetchOptions { Uuids = uuids }, unboundAnchors,
		...
		List<UnboundAnchor> unboundAnchors)
	{
		...
		OVRPlugin.SpaceQueryInfo2 query = OVRSpaceQuery.ForAnchorsThrow(uuids, nameof(uuids));

		unboundAnchors.Clear();

		using (new OVRObjectPool.ListScope<OVRAnchor>(out List<OVRAnchor> anchorBuff))
		{
			OVRPlugin.Result fetchResult = await OVRAnchor.FetchAnchors(anchorBuff, query);

			if (!fetchResult.IsSuccess()) return OVRResult.From(unboundAnchors, (OperationResult)fetchResult);

			foreach (OVRAnchor fetched in anchorBuff)
				if (TryGetUnbound(fetched, out UnboundAnchor unboundAnchor))
		...
		List<UnboundAnchor> unboundAnchors)
	{
		...
		OVRPlugin.SpaceQueryInfo2 query = OVRSpaceQuery.ForGroupThrow(groupUuid, nameof(groupUuid));
		...
		List<UnboundAnchor> unboundAnchors)
	{
		...
		OVRPlugin.SpaceQueryInfo2 groupQuery =
			OVRSpaceQuery.ForGroupThrow(groupUuid, nameof(groupUuid), allowedAnchorUuids);

		using (new OVRObjectPool.ListScope<OVRAnchor>(out List<OVRAnchor> anchorBuff))
		{
			OVRPlugin.Result fetchResult = await OVRAnchor.FetchAnchors(anchorBuff, groupQuery);
		...
		Action<List<UnboundAnchor>, int> resultsHandler)
	{
		...
		using (new OVRObjectPool.ListScope<OVRAnchor>(out List<OVRAnchor> anchors))
		{
			OVRResult<List<OVRAnchor>, OVRAnchor.FetchResult> result = await OVRAnchor.FetchAnchorsAsync(anchors,
				fetchOptions, resultsHandler == null
					? null
					: (incrementalResults, staringIndex) =>
					{
						...
						for (int i = 0; i < incrementalResults.Count; i++)
							if (TryGetUnbound(incrementalResults[i], out UnboundAnchor unboundAnchor))
							{
			...
			unboundAnchors.Clear();
			if (result.Success)
				foreach (OVRAnchor anchor in result.Value)
					if (TryGetUnbound(anchor, out UnboundAnchor unboundAnchor))
						unboundAnchors.Add(unboundAnchor);
		}
	...
	private static bool TryGetUnbound(OVRAnchor anchor, out UnboundAnchor unboundAnchor)
	{
		...
		if (SpatialAnchors.TryGetValue(unboundAnchor.Uuid, out OVRSpatialAnchor alreadyBound))
		{
		...
		bool supportsLocatable = anchor.TryGetComponent<OVRLocatable>(out OVRLocatable locatable);

#if DEVELOPMENT_BUILD || UNITY_EDITOR
		using OVRObjectPool.ListScope<OVRPlugin.SpaceComponentType> _ =
			new(out List<OVRPlugin.SpaceComponentType> supportedComponents);
		anchor.GetSupportedComponents(supportedComponents);
#pragma warning disable OVR004
		string debugComponentList = supportedComponents.Count == 0
			? "(no components)"
...
#pragma warning restore
		...
		{
			Development.LogError(
				$"[{nameof(TryGetUnbound)}] ERROR: {anchor.Uuid:B} does not support {nameof(OVRLocatable)}, and cannot be used as a spatial anchor.");
			return false;
		}
		...
		OVRPlugin.SpaceComponentType componentType, bool enabled)
	{
		...
		if (componentType == OVRPlugin.SpaceComponentType.Locatable &&
		    SpatialAnchors.TryGetValue(uuid, out OVRSpatialAnchor anchor))
	...
	private static class Development
	{
		...
		{
			Debug.Log($"[{nameof(OVRSpatialAnchor)}] {message}");
		}
		...
		{
			Debug.LogWarning($"[{nameof(OVRSpatialAnchor)}] {message}");
		}
		...
		{
			Debug.LogError($"[{nameof(OVRSpatialAnchor)}] {message}");
		}
