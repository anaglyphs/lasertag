...
{
    internal class ImageSegmentationInstallationRoutine : AIBlocksInstallationRoutine
    {
        [SerializeField]
        [Variant(
            Behavior = VariantAttribute.VariantBehavior.Parameter,
            Description = "Whether image segmentation visualizer, which draws the bounding boxes, should also be included.",
            Default = true,
            Order = 100
        )]
        ...
        private static void DisableVisualization(IEnumerable<GameObject> objects)
        {
            ...
            {
                RemoveComponent<ImageSegmentationVisualizer>(@object);
