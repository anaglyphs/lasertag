...
namespace Meshia.MeshSimplification
{
	...
	/// </remarks>
	internal unsafe struct UnsafeMinPriorityQueue<T> : INativeDisposable
		where T : unmanaged, IComparable<T>
	{
		...
		{
			nodes = new UnsafeList<T>(initialCapacity, allocator, options);
		}

		public static UnsafeMinPriorityQueue<T>* Create(int initialCapacity, AllocatorManager.AllocatorHandle allocator,
			NativeArrayOptions options = NativeArrayOptions.UninitializedMemory)
		{
			UnsafeMinPriorityQueue<T>* queue = AllocatorManager.Allocate<UnsafeMinPriorityQueue<T>>(allocator);
			*queue = new UnsafeMinPriorityQueue<T>(initialCapacity, allocator, options);
			return queue;
		}
		...
		{
			CheckNull(queue);
			AllocatorManager.AllocatorHandle allocator = queue->nodes.Allocator;
			queue->Dispose();
		...
		{
			UnsafeMinPriorityQueue<T>* queue = (UnsafeMinPriorityQueue<T>*)list;
			queue->Heapify();
			return queue;
		}
		...
		public void EnqueueRange(ReadOnlySpan<T> elements)
		{
			...
			if (nodes.Capacity < newCount) nodes.SetCapacity(newCount);
			...
			{
				foreach (T element in elements) Enqueue(element);
			}
		}
		...
		private void RemoveRootNode()
		{
			...
			if (lastNodeIndex > 0)
			{
				T lastNode = nodes[lastNodeIndex];
				MoveDown(lastNode, 0);
			}
		...
		private static int GetParentIndex([AssumeRange(1, int.MaxValue)] int index)
		{
		...
		internal void Heapify()
		{
			...
			for (int index = lastParentWithChildren; index >= 0; --index) MoveDown(nodes[index], index);
		}
		...
		private void MoveDown(T node, int nodeIndex)
		{
			...
			while ((i = GetFirstChildIndex(nodeIndex)) < nodes.Length)
			{
				...
				T minChild = nodes[i];
				int minChildIndex = i;
				int childIndexUpperBound = math.min(i + Arity, nodes.Length);
				...
				if (node.CompareTo(minChild) <= 0) break;
		...
		internal static void CheckNull(void* queue)
		{
			if (queue == null)
				throw new InvalidOperationException(
					"UnsafeMinPriorityQueue has yet to be created or has been destroyed!");
		}
	}
}